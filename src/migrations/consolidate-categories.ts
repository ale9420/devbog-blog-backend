...
/**
 * Brings the categories to the five of the redesign. Idempotent: once the
 * targets exist with their slugs and no legacy rows remain, it changes nothing.
...
export async function consolidateCategories(strapi: Core.Strapi): Promise<ConsolidationReport> {
  ...
  for (const target of CATEGORY_TARGETS) {
    ...
    if (keeper) {
      keeperId = keeper.id;
      if (
        keeper.slug !== target.slug ||
        keeper.name !== target.name ||
        keeper.description !== target.description
      ) {
        await categories.update({
          documentId: keeper.documentId,
          data: { slug: target.slug, name: target.name, description: target.description },
        });
